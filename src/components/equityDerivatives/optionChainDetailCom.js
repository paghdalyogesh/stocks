...
class OptionChainDetailCom extends Component {
  constructor(props) {
    super(props);
    this.state = {
      sorts: 1
  ...
  render() {
    ...
    if (list && list.data) {
      dataTableView = list.data.map((data, index) => {
        if (data.CE.lastPrice > 1 && data.PE.lastPrice > 1) { //&& data.CE.strikePrice === 10000
          greekOption(data.CE, index, 'CE');
          greekOption(data.PE, index, 'PE');
          if ((data.CE.gamma >= 0) || (data.PE.gamma >= 0)) {
            return (
              <tr className="pl-0 pb-2 col-md-6 col-lg-6 col-xl-4" key={data.strikePrice}>
                <td>
                  {(data.CE.change < 0 && data.CE.changeinOpenInterest < 0) ? 'BULLISH' : ''}
                  {(data.CE.change > 0 && data.CE.changeinOpenInterest > 0) ? 'BULLISH' : ''}
                  {(data.CE.change > 0 && data.CE.changeinOpenInterest < 0) ? 'BULLISH' : ''}
                  {(data.CE.change < 0 && data.CE.changeinOpenInterest > 0) ? 'BEARISH' : ''}
                </td>
                <td>
                  {(data.CE.change < 0 && data.CE.changeinOpenInterest < 0) ? 'Long Liquidation' : ''}
                  {(data.CE.change < 0 && data.CE.changeinOpenInterest > 0) ? 'Short Buildup' : ''}
                  {(data.CE.change > 0 && data.CE.changeinOpenInterest > 0) ? 'Long Buildup' : ''}
                  {(data.CE.change > 0 && data.CE.changeinOpenInterest < 0) ? 'Short covering' : ''}
                </td>
                <td>{data.CE.changeinOpenInterest > 0 ? 'UP' : 'DOWN'}</td>
                <td>{data.CE.change > 0 ? 'UP' : 'DOWN'}</td>
                ...
                <td>{data.PE.change > 0 ? 'UP' : 'DOWN'}</td>
                <td>{data.PE.changeinOpenInterest > 0 ? 'UP' : 'DOWN'}</td>
                <td>
                  {(data.PE.change < 0 && data.PE.changeinOpenInterest < 0) ? 'Long Liquidation' : ''}
                  {(data.PE.change < 0 && data.PE.changeinOpenInterest > 0) ? 'Short Buildup' : ''}
                  {(data.PE.change > 0 && data.PE.changeinOpenInterest > 0) ? 'Long Buildup' : ''}
                  {(data.PE.change > 0 && data.PE.changeinOpenInterest < 0) ? 'Short covering' : ''}
                </td>
                <td>
                  {(data.PE.change < 0 && data.PE.changeinOpenInterest < 0) ? 'BULLISH' : ''}
                  {(data.PE.change > 0 && data.PE.changeinOpenInterest > 0) ? 'BULLISH' : ''}
                  {(data.PE.change > 0 && data.PE.changeinOpenInterest < 0) ? 'BULLISH' : ''}
                  {(data.PE.change < 0 && data.PE.changeinOpenInterest > 0) ? 'BEARISH' : ''}
                </td>
              </tr>
            )
          }
...
export default connect(mapStatetoProps, mapDispatchToProps)(OptionChainDetailCom);
